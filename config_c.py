...
while (linkCounter < linkList.contents.value):
    ...
    if linkCounter != (indexReturn1.contents.value or indexReturn2.contents.value):
        if (int(currentRough.contents.value) > 140):
            randironAge = np.random.rand(0,75)
            ...
            if (linkCounter > 20):
                randpvcAge = np.random.rand(27, 6)
            else:
                randpvcAge = float(np.random.normal(13, 3))
...
tH1 = np.random.rand(0, 1)
tH2 = np.random.rand(0, 1)
