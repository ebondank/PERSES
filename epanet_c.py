    ...
    while epaCount < 144:
        ...
        for index, item in enumerate(data[simType]['pvc']):
            ...
            if (int(data[simType]['pvc']['fS'][index + 1]) != 0):
                ...
                if (int(data[simType]['pvc']['fS'][index + 1]) <= 0):
                    # pipe enable
                    parent_c.epalib.ENsetlinkvalue(data[simType]['pvc']['index'][index + 1], ct.c_int(11), ct.c_float(1))
                    # no-time simulation config stuff
                    if (simType != 'noTime'):
                        pvcPipeAges[simType][index + 1] = 0
                # Pipe disable mid run
                else:
                    parent_c.epalib.ENsetlinkvalue(data[simType]['pvc']['index'][index + 1], ct.c_int(11), ct.c_float(0.0))
            else:
                indexSelect = 0
        ...
        for index, item in enumerate(ironPipesList):
            if (int(data[simType]['iron']['fS'][index + 1]) != 0):
                ...
                if (int(data[simType]['iron']['fS'][index + 1]) <= 0):
                    parent_c.epalib.ENsetlinkvalue(data[simType]['iron']['index'][index + 1], ct.c_int(11), ct.c_float(1))
                else:
                    parent_c.epalib.ENsetlinkvalue(data[simType]['iron']['index'][index + 1], ct.c_int(11), ct.c_float(0.0))
                if (simType != 'noTime'):
        ...
        for index, item in enumerate(pumpList):
            if (int(data[simType]['pump']['fS'][index + 1]) != 0):
                ...
                parent_c.epalib.ENsetlinkvalue(data[simType]['pump']['index'][index + 1], ct.c_int(12), ct.c_float(0.0))
                if (int(data[simType]['pump']['fS'][index + 1]) <= 0):
                    parent_c.epalib.ENsetlinkvalue(data[simType]['pump']['index'][index + 1], ct.c_int(12), ct.c_float(1.0))
                    if (simType != 'noTime'):
        ...
        errorcode = parent_c.parent_c.epalib.ENrunH(time)
        ...
        parent_c.parent_c.epalib.ENnextH(parent_c.timestep)
